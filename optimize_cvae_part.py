...
parser.add_argument('--recon_model', type=str, required=True)
...
def optimize(args):
    ...
    model_cvae.load_state_dict(torch.load(args.prior_model, map_location=device))
    model_cvae.eval()

    model_vox = VoxelPredNet(hidden_size=args.recon_hidden).to(device)
    model_vox.load_state_dict(torch.load(args.recon_model))
    model_vox.eval()

    train_dataset = AhoiDatasetGlobal(data_folder='/home/jiangnan/AHOI/temp_data/data_blocks_train', n_grid=args.n_grid, add_human=True)
    ...
    for i, (output, idx) in enumerate(train_loader):
        ...
        if args.save_image:
            for _ in range(10):
                images.append(img)
            images[0].save(os.path.join(args.output_dir, f'vis_{global_ind:02d}.gif'), save_all=True, append_images=images[1:], optimize=False, duration=100, loop=0 )
